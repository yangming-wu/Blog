...
const Base = class SimpleCalendar {
  //构造函数
  ...
  create() {
    ...
    setInterval(function() {
      ...
      [].forEach.call(timespan, function(v) {
        v.innerHTML =  nh + ":" + nm + ':' + ns;  // 时间 ： xx:xx:xx
      })
    }, 200);
  }
  ...
  addData(year, month) {
    ...
    for (var i = 0; i < 35; i++) {
      ...
      if (this._options.showLunarCalendar) {
        daysElement[i].querySelector('.lunar-day').innerHTML = (new Lunar(writeyear, writemonth, writeday)).getLunarDayName();
  ...
  updateFestival(year, month) {
    ...
    daysElement.forEach(function(v, i) {
      ...
      if (lunardata && options.showLunarFestival) {
        var lunar =(new Lunar(year, currentmonth, day)).getLunarDayNum();
...
Base.prototype.tyear = (new Date()).getFullYear();
Base.prototype.tmonth = (new Date()).getMonth() + 1;
Base.prototype.tday = (new Date()).getDate();

let initCalendar = function(id){
  return new Base(id);
}
